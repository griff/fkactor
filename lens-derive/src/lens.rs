...
fn derive_struct(input: &syn::DeriveInput) -> Result<proc_macro2::TokenStream, syn::Error> {
    ...
    let impls = fields.iter().map(|f| {
        ...
        quote! {
            impl fkactor::Lens<#ty, #field_ty> for #twizzled_name::#field_name {
            ...
            impl fkactor::AsyncLens<#ty, #field_ty> for #twizzled_name::#field_name {
                fn with<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data #ty, f: F) -> std::pin::Pin<Box<dyn std::future::Future<Output=V> + Send + 'async_trait>>
                    where 'this: 'async_trait,
                          'data: 'async_trait,
                        V: 'async_trait,
                            Fut: std::future::Future<Output=V> + Send + 'async_trait,
                            F: (FnOnce(&'data #field_ty) -> Fut) + Send + 'async_trait
                {
                    Box::pin(f(&data.#field_name))
                }
        
                fn with_mut<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data mut #ty, f: F) -> std::pin::Pin<Box<dyn std::future::Future<Output=V> + Send + 'async_trait>>
                    where 'this: 'async_trait,
                          'data: 'async_trait,
                            V: 'async_trait,
                            Fut: std::future::Future<Output=V> + Send + 'async_trait,
                            F: (FnOnce(&'data mut #field_ty) -> Fut) + Send + 'async_trait
                {
                    Box::pin(f(&mut data.#field_name))
                }
            }
        }
    });
