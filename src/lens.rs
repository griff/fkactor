...
impl<T, U, B: ?Sized> Then<T, U, B> {
    ...
    pub fn new_async<A: ?Sized, C: ?Sized>(left: T, right: U) -> Self
    where
        T: AsyncLens<A, B>,
        U: AsyncLens<B, C>,
    {
...
impl<L> InArc<L> {
    ...
    pub fn new<A, B: ?Sized>(inner: L) -> Self
    where
        A: Clone,
        B: Eq + Clone,
        L: Lens<A, B>,
    ...
    L: Lens<A, B>,
{
    ...
    fn with_mut<V, F: FnOnce(&mut B) -> V>(&self, data: &mut Arc<A>, f: F) -> V {
        ...
        let v = f(&mut temp);
        if self.inner.with(data, |x| temp != *x) {
...
pub trait AsyncLens<T: ?Sized, U: ?Sized>
{
    fn with<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data T, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
        where 'this: 'async_trait,
              ...
              V: 'async_trait,
                ...
                F: (FnOnce(&'data U) -> Fut) + Send + 'async_trait;

    fn with_mut<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data mut T, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
        where 'this: 'async_trait,
              'data: 'async_trait,
                U: 'data,
                V: 'async_trait,
                Fut: Future<Output=V> + Send + 'async_trait,
                F: (FnOnce(&'data mut U) -> Fut) + Send + 'async_trait;

}

/// Helpers for manipulating `Lens`es
pub trait AsyncLensExt<A: ?Sized, B: ?Sized>: AsyncLens<A, B> {
    /// Copy the targeted value out of `data`
    fn get<'this, 'data, 'async_trait>(&'this self, data: &'data A) -> Pin<Box<dyn Future<Output=B> + Send + 'async_trait>>
    where
        'this: 'async_trait,
        'data: 'async_trait,
        B: Clone + Send + 'async_trait,
    {
        self.with(data, |x| ready(x.clone()) )
    }

    /// Set the targeted value in `data` to `value`
    fn put<'this, 'data, 'async_trait>(&'this self, data: &'data mut A, value: B) -> Pin<Box<dyn Future<Output=()> + Send + 'async_trait>>
    where
        'this: 'async_trait,
        'data: 'async_trait,
        B: Sized + Send + 'async_trait,
    {
        self.with_mut(data, move |x| { *x = value; ready(()) })
    }
    ...
    fn then<Other, C>(self, other: Other) -> Then<Self, Other, B>
    where
        Other: AsyncLens<B, C> + Sized,
        C: ?Sized,
        Self: Sized,
    {
        Then::new_async(self, other)
    }
...
impl<A: ?Sized, B: ?Sized, T: AsyncLens<A, B>> AsyncLensExt<A, B> for T {}


impl<T, U, Get, GetMut> AsyncLens<T, U> for Field<Get, GetMut>
where
    T: ?Sized,
    U: ?Sized,
    Get: Fn(&T) -> &U,
    ...
    fn with_mut<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data mut T, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
        where 'this: 'async_trait,
              'data: 'async_trait,
                ...
                F: (FnOnce(&'data mut U) -> Fut) + Send + 'async_trait
    {
        Box::pin(f((self.get_mut)(data)))
    }
}

impl<T, U, A, B, C> AsyncLens<A, C> for Then<T, U, B>
where
    A: ?Sized,
    B: ?Sized + Sync + 'static,
    C: ?Sized,
    T: AsyncLens<A, B> + Sync,
    U: AsyncLens<B, C> + Sync,
{
    fn with<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data A, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
        where 'this: 'async_trait,
              'data: 'async_trait,
              A: 'data,
              C: 'data,
              V: 'async_trait,
                Fut: Future<Output=V> + Send + 'async_trait,
                F: (FnOnce(&'data C) -> Fut) + Send + 'async_trait
    {
        self.left.with(data, move |b| self.right.with(b, f) )
    }

    fn with_mut<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data mut A, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
        where 'this: 'async_trait,
              'data: 'async_trait,
                C: 'data,
                V: 'async_trait,
                Fut: Future<Output=V> + Send + 'async_trait,
                F: (FnOnce(&'data mut C) -> Fut) + Send + 'async_trait
    {
        self.left.with_mut(data, move |b| self.right.with_mut(b, f))
    }

}

/*
impl<A: ?Sized, B, Get, Put, D> AsyncLens<A, B> for Map<Get, Put>
where
    A: Send + Sync,
    ...
    Put: (Fn(D, B)) + Sync,
{
    fn with<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: D, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
        where 'this: 'async_trait,
              'data: 'async_trait,
              //'fdata: 'async_trait,
               D: Borrow<A> + Send + 'data,
                A: 'data,
                B: 'async_trait,
                Fut: Future<Output=V> + Send + 'async_trait,
                F: (FnOnce(& 'async_trait B) -> Fut) + Send + 'async_trait,
    {
        let fut = async move {
            let v = (self.get)(data);
            let vb : &'async_trait B = &v;
            let x = f(vb).await;
            x
        };
        Box::pin(fut)
    ...
    fn with_mut<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: D, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
        where 'this: 'async_trait,
              'data: 'async_trait,
                B: 'data,
                Fut: Future<Output=V> + Send + 'async_trait,
                F: (FnOnce(&'data mut B) -> Fut) + Send + 'async_trait,
    {
        let fut = async move {
            let mut temp = (self.get)(data);
            let fu = f(&mut temp);
...
impl<T: ?Sized> AsyncLens<T, T::Target> for Deref
where
    T: ops::Deref + ops::DerefMut,
{
    fn with<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data T, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
        where 'this: 'async_trait,
              ...
              T::Target: 'data,
                Fut: Future<Output=V> + Send + 'async_trait,
                F: (FnOnce(&'data T::Target) -> Fut) + Send
    {
        Box::pin(f(data.deref()))
    }
    fn with_mut<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data mut T, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
        where 'this: 'async_trait,
              'data: 'async_trait,
                T::Target: 'data,
                Fut: Future<Output=V> + Send + 'async_trait,
                F: (FnOnce(&'data mut T::Target) -> Fut) + Send
    {
        Box::pin(f(data.deref_mut()))
    }
}

impl<T, I> AsyncLens<T, T::Output> for Index<I>
where
    T: ?Sized + ops::Index<I> + ops::IndexMut<I>,
    I: Clone,
{
    fn with<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data T, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
        where 'this: 'async_trait,
              'data: 'async_trait,
                T::Output: 'data,
                Fut: Future<Output=V> + Send + 'async_trait,
                F: (FnOnce(&'data T::Output) -> Fut) + Send
    {
        Box::pin(f(&data[self.index.clone()]))
    }
    ...
    fn with_mut<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data mut T, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
        where 'this: 'async_trait,
              'data: 'async_trait,
                T::Output: 'data,
                Fut: Future<Output=V> + Send + 'async_trait,
                F: (FnOnce(&'data mut T::Output) -> Fut) + Send
    {
        Box::pin(f(&mut data[self.index.clone()]))
    }
}

impl<A: ?Sized> AsyncLens<A, A>  for Id
    where A: Send,
{
    fn with<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data A, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
        where 'this: 'async_trait,
...
impl<S: ?Sized> AsyncLens<Arc<Mutex<S>>, S>  for MutexLock
    where S: Send + Sync,
{
    fn with<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data Arc<Mutex<S>>, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
    where 'this: 'async_trait,
          'data: 'async_trait,
          S: 'data,
          V: 'async_trait,
            Fut: Future<Output=V> + Send + 'async_trait,
            F: (FnOnce(&'data S) -> Fut) + Send + 'async_trait
    {
        let fut = async move {
            let s = data.lock().await;
            f(ops::Deref::deref(&s)).await
        };
        Box::pin(fut)
    }

    fn with_mut<'this, 'data, 'async_trait, V, Fut, F>(&'this self, data: &'data mut Arc<Mutex<S>>, f: F) -> Pin<Box<dyn Future<Output=V> + Send + 'async_trait>>
        where 'this: 'async_trait,
              'data: 'async_trait,
                S: 'data,
                V: 'async_trait,
                Fut: Future<Output=V> + Send + 'async_trait,
                F: (FnOnce(&'data mut S) -> Fut) + Send + 'async_trait
    {
        let fut = async move {
            let s = data.lock().await;
            f(ops::DerefMut::deref_mut(&mut s)).await
        };
        Box::pin(fut)
    }
}
